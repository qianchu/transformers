...
""" Finetuning multi-lingual models on Xlwic (Bert, DistilBERT, XLM).
...
from transformers import xlwic_convert_examples_to_features as convert_examples_to_features
from transformers import xlwic_compute_metrics as compute_metrics
from transformers import xlwic_output_modes as output_modes
from transformers import xlwic_processors as processors
...
def find_token_id(input_id,tokenizer):
    ...
    try:
        assert len(token_ids_alter)==1
    except AssertionError as e:
        print ('Warning, token id alter is not length 1')
        print (input_id)
    ...
    if len(token_ids)<1:
        ...
        if len(token_ids)>1:
            print (input_id)
            print('[ token id ',token_pos_start_id)
            print('Warning: more than one [',token_ids)
...
def train(args, train_dataset, model, tokenizer):
    ...
    if args.should_continue and os.path.isfile(os.path.join(args.model_name_or_path, "optimizer.pt")) and os.path.isfile(
    ...
    if args.should_continue and os.path.exists(args.model_name_or_path):
...
def load_and_cache_examples(args, task, tokenizer, evaluate=False,testset='test'):
    ...
    dataset = TensorDataset(all_input_ids_a, all_attention_mask_a, all_token_ids_a,all_input_ids_b,all_attention_mask_b,all_token_ids_b,all_labels)
    return dataset
...
def main():
    ...
    parser.add_argument(
        "--should_continue",
        action='store_true',
        help="whether to continue from pretrained model"
    )
    parser.add_argument(
        "--model_type",
        default=None,
