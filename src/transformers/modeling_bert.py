...
class BertForSequenceTokenClassification(BertPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
    ...
    Examples::
        ...
        pooled_output=torch.cat((outputs[1],pooled_output_w1,pooled_output_w2),1)
...
class BertForSequenceTokenLeftClassification(BertPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
        self.num_labels = config.num_labels
    ...
    Examples::
        ...
        try:
            pooled_output = outputs[0][range(outputs[0].size()[0]),token_ids[:,0],:]
        except TypeError as e:
            print ('Warning: Nonetype token ids',e)
            print (outputs[0].size())
            print (token_ids)
