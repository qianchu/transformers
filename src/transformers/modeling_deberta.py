...
@add_start_docstrings(
    """
    DeBERTa Model transformer with a sequence token classification/regression head on top (a linear layer on top of the
    pooled output) e.g. for GLUE tasks.
    """,
    DEBERTA_START_DOCSTRING,
)
class DebertaForSequenceTokenClassification(DebertaPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
        ...
        self.classifier = torch.nn.Linear(output_dim*3, num_labels)
        drop_out = getattr(config, "cls_dropout", None)
        drop_out = self.config.hidden_dropout_prob if drop_out is None else drop_out
        self.dropout = StableDropout(drop_out)
    ...
    def forward(
        ...
        pooled_output=torch.cat((self.pooler(outputs[0]),pooled_output_w1,pooled_output_w2),1)
        pooled_output = self.dropout(pooled_output)
