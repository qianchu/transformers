...
MODEL_FOR_PARA_ALIGN_MAPPING = OrderedDict(
    [
        (XLMRobertaConfig, XLMRobertaForParaAlign),
...
class AutoModelForParaAlign:
    r"""
        :class:`~transformers.AutoModelForParaAlign` is a generic model class
        that will be instantiated as one of the sequence classification model classes of the library
        when created with the `AutoModelForParaAlign.from_pretrained(pretrained_model_name_or_path)`
        class method.
    ...
    def from_config(cls, config):
        ...
        Examples::
            ...
            model = AutoModelForSequenceClassification.from_config(config)  # E.g. model was saved using `save_pretrained('./test/saved_model/')`
        """
        for config_class, model_class in MODEL_FOR_PARA_ALIGN_MAPPING.items():
            if isinstance(config, config_class):
                return model_class(config)
        raise ValueError(
            "Unrecognized configuration class {} for this kind of AutoModel: {}.\n"
            "Model type should be one of {}.".format(
                config.__class__,
                cls.__name__,
                ", ".join(c.__name__ for c in MODEL_FOR_PARA_ALIGN_MAPPING.keys()),
            )
        )
